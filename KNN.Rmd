...
data_set = merge(X, y, by="id")
data_set = subset(data_set, select=-c(id, X))
data_set_reduce = subset(data_set, select=-c(region, scheme_management))

#This part keeps erroring
model_lvq = train(status_group~., data = data_set_reduce, 
                  method="lvq", preProcess="scale", trControl=ctrl)
